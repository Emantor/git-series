...
fn reference_matching_opt<'repo>(
    ...
    match current_id_opt {
        ...
        Some(current_id) => Ok(try!(repo.reference_matching(
            name,
            id,
            force,
            current_id,
...
fn parents_from_ids(repo: &Repository, mut parents: Vec<Oid>) -> Result<Vec<Commit>> {
    ...
    parents
        .drain(..)
        .map(|id| Ok(try!(repo.find_commit(id))))
...
impl<'repo> Internals<'repo> {
    ...
    fn read_series(repo: &'repo Repository, series_name: &str) -> Result<Self> {
        let committed_id = try!(notfound_to_none(repo.refname_to_id(&format!(
            "{}{}",
            SERIES_PREFIX, series_name
        ))));
        let maybe_get_ref = |prefix: &str| -> Result<TreeBuilder<'repo>> {
            match try!(notfound_to_none(
                repo.refname_to_id(&format!("{}{}", prefix, series_name))
            )).or(committed_id)
            {
                Some(id) => {
    ...
    fn copy(repo: &'repo Repository, source: &str, dest: &str) -> Result<bool> {
        ...
        for prefix in [SERIES_PREFIX, STAGED_PREFIX, WORKING_PREFIX].iter() {
            ...
            if let Some(r) = try!(notfound_to_none(repo.find_reference(&prefixed_source))) {
                let oid = try!(r.target().ok_or(format!(
    ...
    fn update_series(&mut self, repo: &'repo Repository) -> Result<()> {
        ...
        self.working
            .insert("series", head_id, GIT_FILEMODE_COMMIT as i32)?;
        Ok(())
    }
    ...
    fn write(&self, repo: &'repo Repository) -> Result<()> {
        ...
        let maybe_commit = |prefix: &str, tb: &TreeBuilder| -> Result<()> {
            ...
            let commit_id =
                try!(repo.commit(None, &author, &committer, &refname, &tree, &parents_ref));
            try!(repo.reference_ensure_log(&refname));
            try!(reference_matching_opt(
                repo,
                &refname,
                commit_id,
                true,
...
fn series(out: &mut Output, repo: &Repository) -> Result<()> {
    ...
    for name in refs.iter() {
        ...
        let new = if try!(notfound_to_none(
            repo.refname_to_id(&format!("{}{}", SERIES_PREFIX, name))
        )).is_none()
        {
            " (new, no commits yet)"
        } else {
    ...
    if refs.is_empty() {
        try!(writeln!(
            out,
            "No series; use \"git series start <name>\" to start"
        ));
    }
    Ok(())
}
...
fn start(repo: &Repository, m: &ArgMatches) -> Result<()> {
    ...
    try!(repo.reference(
        ...
        true,
        &format!(
            "checkout: moving from {} to {} (git series start {})",
            head_id, head_id, name
        )
    ));
    println!(
        "HEAD is now detached at {}",
        try!(commit_summarize(&repo, head_id))
    );
    Ok(())
}
...
fn checkout(repo: &Repository, m: &ArgMatches) -> Result<()> {
    ...
    let new_head_id = try!(
        try!(internals.working.get("series"))
    ...
    try!(repo.reference(
        ...
        true,
        &format!(
            "checkout: moving from {} to {} (git series checkout {})",
            head_id, new_head_id, name
        )
    ));
    println!(
        "HEAD is now detached at {}",
        try!(commit_summarize(&repo, new_head_id))
    );
...
fn base(repo: &Repository, m: &ArgMatches) -> Result<()> {
    ...
    if !current_base_id.is_zero() {
        println!(
            "Previous base was {}",
            try!(commit_summarize(&repo, current_base_id))
        );
    }
    ...
    if new_base_id.is_zero() {
        ...
        try!(
            internals
                .working
                .insert("base", new_base_id, GIT_FILEMODE_COMMIT as i32)
        );
        try!(internals.write(repo));
        println!(
            "Set patch series base to {}",
            try!(commit_summarize(&repo, new_base_id))
        );
    }
...
fn get_pager(config: &Config, for_cmd: &str, default: bool) -> Option<OsString> {
    ...
    pager.and_then(|p| {
        if p.is_empty() || p == OsString::from("cat") {
            None
        } else {
            Some(p)
        }
...
fn cmd_maybe_shell<S: AsRef<OsStr>>(program: S, args: bool) -> Command {
    if program
        .as_ref()
        .to_string_lossy()
        .contains(|c| SHELL_METACHARS.contains(c))
    {
...
impl Output {
    ...
    fn get_color(
        ...
        let color_cmd = try!(notfound_to_none(
            config.get_str(&format!("color.{}", command))
        )).unwrap_or(color_ui);
        ...
        if self.pager.is_some() {
            let color_pager =
                try!(notfound_to_none(config.get_bool(&format!("color.pager")))).unwrap_or(true);
            if !color_pager {
                return Ok(Style::new());
            }
...
fn commit_status(
    ...
    let (changes, tree) = if commit_all {
        ...
        let changes_to_be_committed = try!(write_status(
            &mut status,
            &diff,
            "Changes to be committed:",
            &color_updated,
        ...
        let diff_not_staged =
            try!(repo.diff_tree_to_tree(Some(&staged_tree), Some(&working_tree), None));
        let changes_not_staged = try!(write_status(
            &mut status,
            &diff_not_staged,
    ...
    if let Some(base) = tree.get_name("base") {
        if base.id() != series_id && !try!(repo.graph_descendant_of(series_id, base.id())) {
            ...
            let (series_short_id, series_summary) =
                try!(commit_summarize_components(&repo, series_id));
            return Err(format!(
                concat!(
    ...
    let new_commit_oid = try!(repo.commit(
        Some(SHEAD_REF),
        &author,
        &committer,
        &msg,
    ...
    let (new_commit_short_id, new_commit_summary) =
        try!(commit_summarize_components(&repo, new_commit_oid));
    try!(writeln!(
        out,
        "[{} {}] {}",
        series_name, new_commit_short_id, new_commit_summary
    ));
...
fn cover(repo: &Repository, m: &ArgMatches) -> Result<()> {
    ...
    let (working_cover_id, working_cover_content) = match try!(internals.working.get("cover")) {
        ...
        Some(entry) => (
            entry.id(),
            try!(std::str::from_utf8(
                try!(repo.find_blob(entry.id())).content()
            )).to_string(),
        ),
    };
    ...
    if new_cover_id == working_cover_id {
        ...
        try!(
            internals
                .working
                .insert("cover", new_cover_id, GIT_FILEMODE_BLOB as i32)
        );
        try!(internals.write(repo));
...
fn diffstat(diff: &Diff) -> Result<String> {
    ...
    let stats_buf =
        try!(stats.to_buf(git2::DIFF_STATS_FULL | git2::DIFF_STATS_INCLUDE_SUMMARY, 72));
...
fn write_commit_range_diff<W: IoWrite>(
    ...
    for commit in commits1.iter().chain(commits2.iter()) {
        if commit.parent_ids().count() > 1 {
            try!(writeln!(
                out,
                "(Diffs of series with merge commits ({}) not yet supported)",
                commit.id()
            ));
            return Ok(());
        }
    }
    let ncommon = commits1
        .iter()
        .zip(commits2.iter())
    ...
    for i1 in 0..ncommits1 {
        for i2 in 0..ncommits2 {
            let patch = try!(git2::Patch::from_buffers(
                &texts1[i1].0,
                None,
                &texts2[i2].0,
                None,
    ...
    let mut commits1_state: Vec<_> = std::iter::repeat(CommitState::Unhandled)
        .take(ncommits1)
        .collect();
    ...
    let nwidth = max(ncommits1 + offset, ncommits2 + offset)
        .to_string()
        .len();
    let commits1_summaries: Vec<_> = try!(
        commits1
            .iter_mut()
            .map(commit_obj_summarize_components)
            .collect()
    );
    let commits2_summaries: Vec<_> = try!(
        commits2
            .iter_mut()
            .map(commit_obj_summarize_components)
            .collect()
    );
    let idwidth = commits1_summaries
        .iter()
        .chain(commits2_summaries.iter())
    ...
    for commit_pair in commit_pairs {
        match commit_pair {
            ...
            (Some(i1), Some(i2)) => {
                let mut patch = try!(git2::Patch::from_buffers(
                    &texts1[i1].0,
                    None,
                    &texts2[i2].0,
                    None,
                ...
                v.push(
                    colors
                        .commit
                        .paint(format!(" {} ", ch).as_bytes().to_owned()),
                );
                v.push(
                    new.paint(
                        format!(
                ...
                v.push(
                    colors
                        .commit
                        .paint(format!(" {}", c2_summary).as_bytes().to_owned()),
                );
                nl(&mut v);
...
fn write_series_diff<W: IoWrite>(
    ...
    if let (Some(base1), Some(series1), Some(base2), Some(series2)) =
        (base1, series1, base2, series2)
    {
        try!(write_commit_range_diff(
            out,
            repo,
            colors,
            (base1.id(), series1.id()),
        ...
        try!(writeln!(
            out,
            "Can't diff series: both versions must have base and series to diff"
        ));
    }
...
fn format(out: &mut Output, repo: &Repository, m: &ArgMatches) -> Result<()> {
    ...
    let mut commits: Vec<Commit> = try!(
        revwalk
            .map(|c| {
                let id = try!(c);
                let commit = try!(repo.find_commit(id));
    ...
    let subject_patch = version.map_or(subject_prefix.to_string(), |n| {
        format!("{}{}v{}", subject_prefix, ensure_space(&subject_prefix), n)
    });
    let file_prefix = version.map_or("".to_string(), |n| format!("v{}-", n));

    let directory = m.value_of("output-directory")
    ...
    for (commit_num, commit) in commits.iter().enumerate() {
        ...
        if !no_from
            && (commit_author_name != committer_name || commit_author_email != committer_email)
        {
            try!(writeln!(
                out,
                "From: {} <{}>\n",
...
fn log(out: &mut Output, repo: &Repository, m: &ArgMatches) -> Result<()> {
    ...
    for oid in revwalk {
        ...
        try!(writeln!(
            out,
            "{}",
            diffcolors.commit.paint(format!("commit {}", oid))
        ));
        try!(writeln!(
            out,
            "Author: {} <{}>",
        ...
        if show_diff {
            ...
            if parent_ids.len() > 1 {
                try!(writeln!(
                    out,
                    "(Diffs of series merge commits not yet supported)"
                ));
            } else {
...
fn rebase(repo: &Repository, m: &ArgMatches) -> Result<()> {
    ...
    let base = try!(
        try!(internals.working.get("base"))
    ...
    if !diff_empty(&try!(repo.diff_tree_to_index(
        Some(&series_tree),
        None,
        None
    ))) {
            ...
            .map(|c| {
                ...
                if commit.parent_ids().count() > 1 {
                    return Err(format!(
                        ...
                        try!(commit_obj_summarize(&mut commit))
                    ).into());
                }
                Ok(commit)
            })
    ...
    try!(writeln!(
        git_rebase_todo,
        "\n# Rebase {}..{} onto {}",
        base_short, series_short, newbase_short
    ));
...
fn req(out: &mut Output, repo: &Repository, m: &ArgMatches) -> Result<()> {
    ...
    let (msg, extra_body, remote_pull_name) = match (
        ...
        (Some(remote_tag), Some(remote_tag_peeled), _) => {
            ...
            let local_tag = try!(repo.find_tag(remote_tag).map_err(|e| format!(
    ...
    let mut commits: Vec<Commit> = try!(
        revwalk
            .map(|c| Ok(try!(repo.find_commit(try!(c)))))
            .collect::<Result<_>>()
    );
    if commits.is_empty() {
