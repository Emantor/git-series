...
fn reference_matching_opt<'repo>(
    ...
    match current_id_opt {
        None => Ok(repo.reference(name, id, force, log_message)?),
        Some(current_id) => Ok(repo.reference_matching(name, id, force, current_id, log_message)?),
    }
}
...
impl<'repo> Internals<'repo> {
    ...
    fn read_series(repo: &'repo Repository, series_name: &str) -> Result<Self> {
        let committed_id =
            notfound_to_none(repo.refname_to_id(&format!("{}{}", SERIES_PREFIX, series_name)))?;
        let maybe_get_ref = |prefix: &str| -> Result<TreeBuilder<'repo>> {
            match notfound_to_none(repo.refname_to_id(&format!("{}{}", prefix, series_name)))?
                .or(committed_id)
            {
                Some(id) => {
...
fn unadd(repo: &Repository, m: &ArgMatches) -> Result<()> {
    ...
    let mut internals = Internals::read(repo)?;
    if started {
        let shead_commit = peel_to_commit(shead)?;
...
fn series(out: &mut Output, repo: &Repository) -> Result<()> {
    ...
    for name in &refs {
        ...
        let new = if notfound_to_none(repo.refname_to_id(&format!("{}{}", SERIES_PREFIX, name)))?
            .is_none()
        {
            " (new, no commits yet)"
        } else {
...
fn start(repo: &Repository, m: &ArgMatches) -> Result<()> {
    let head = repo.head()?;
    let head_commit = peel_to_commit(head)?;
...
fn checkout(repo: &Repository, m: &ArgMatches) -> Result<()> {
    ...
    println!(
        "HEAD is now detached at {}",
        commit_summarize(&repo, new_head_id)?
    );
...
fn base(repo: &Repository, m: &ArgMatches) -> Result<()> {
    ...
    if !current_base_id.is_zero() {
        println!(
            "Previous base was {}",
            commit_summarize(&repo, current_base_id)?
        );
    }
    ...
    if new_base_id.is_zero() {
        ...
        println!(
            ...
            commit_summarize(&repo, new_base_id)?
        );
    }
...
fn do_diff(out: &mut Output, repo: &Repository, m: &ArgMatches) -> Result<()> {
    ...
    let from_tree = match m.value_of("From-Commit") {
        Some(revstr) => revparse_to_commit(repo, revstr)?.tree(),
        None if cached => {
            let shead = repo.find_reference(SHEAD_REF)?;
            peel_to_commit(shead.resolve()?)?.tree()
        }
        None => repo.find_tree(Internals::read(&repo)?.staged.write()?),
    }?;
    let to_tree = match m.value_of("To-Commit") {
        Some(revstr) => revparse_to_commit(repo, revstr)?.tree(),
        None if cached => repo.find_tree(Internals::read(&repo)?.staged.write()?),
        None => repo.find_tree(Internals::read(&repo)?.working.write()?),
    }?;
...
impl Output {
    ...
    fn get_color(
        ...
        let color_cmd =
            notfound_to_none(config.get_str(&format!("color.{}", command)))?.unwrap_or(color_ui);
...
fn commit_status(
    ...
    let (changes, tree) = if commit_all {
        ...
        let diff_not_staged =
            repo.diff_tree_to_tree(Some(&staged_tree), Some(&working_tree), None)?;
    ...
    let (new_commit_short_id, new_commit_summary) =
        commit_summarize_components(&repo, new_commit_oid)?;
    writeln!(
        out,
        "[{} {}] {}",
...
fn cover(repo: &Repository, m: &ArgMatches) -> Result<()> {
    ...
    let (working_cover_id, working_cover_content) = match internals.working.get("cover")? {
        ...
        Some(entry) => (
            entry.id(),
            std::str::from_utf8(repo.find_blob(entry.id())?.content())?.to_string(),
        ),
    };
...
fn diffstat(diff: &Diff) -> Result<String> {
    ...
    let stats_buf = stats.to_buf(git2::DIFF_STATS_FULL | git2::DIFF_STATS_INCLUDE_SUMMARY, 72)?;
...
fn write_commit_range_diff<W: IoWrite>(
    ...
    let nwidth = max(ncommits1 + offset, ncommits2 + offset)
        .to_string()
        .len();
    let commits1_summaries = commits1
        .iter_mut()
        .map(commit_obj_summarize_components)
        .collect::<Result<Vec<_>>>()?;
    let commits2_summaries = commits2
        .iter_mut()
        .map(commit_obj_summarize_components)
        .collect::<Result<Vec<_>>>()?;
    ...
    for commit_pair in commit_pairs {
        match commit_pair {
            ...
            (Some(i1), Some(i2)) => {
                let mut patch =
                    git2::Patch::from_buffers(&texts1[i1].0, None, &texts2[i2].0, None, None)?;
    ...
    for mut commit in commits {
        ...
        if let Some(ref cmds) = exec_cmds {
            for cmd in cmds.clone() {
                writeln!(git_rebase_todo, "exec {}", cmd)?;
            }
        }
    }
...
fn req(out: &mut Output, repo: &Repository, m: &ArgMatches) -> Result<()> {
    let config = repo.config()?.snapshot()?;
    let shead = repo.find_reference(SHEAD_REF)?;
    let shead_commit = peel_to_commit(shead.resolve()?)?;
    let stree = shead_commit.tree()?;
